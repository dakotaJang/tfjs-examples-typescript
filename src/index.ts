...
async function train() {
  ...
  for (let i = 0; i < TRAIN_BATCHES; i++) {
    ...
    // with batches.
    const config: tf.ModelFitConfig = Object.assign({batchSize: BATCH_SIZE, validationData, epochs: 1});
    const history = await model.fit(
        batch.xs.reshape([BATCH_SIZE, 28, 28, 1]), batch.labels, config);
...
async function showPredictions() {
  ...
  tf.tidy(() => {
    ...
    const predictions = Array.from((output as tf.Tensor).argMax(axis).dataSync());
