...
const a: tf.Variable = tf.variable(tf.scalar(Math.random()));
const b: tf.Variable = tf.variable(tf.scalar(Math.random()));
const c: tf.Variable = tf.variable(tf.scalar(Math.random()));
const d: tf.Variable = tf.variable(tf.scalar(Math.random()));

// Step 2. Create an optimizer, we will use this later. You can play
// with some of these values to see how the model perfoms.
const numIterations: number = 75;
 ...
 */
async function train(xs: tf.Tensor, ys: tf.Tensor, iterations: number) {
  for (let iter = 0; iter < iterations; iter++) {
    ...
    optimizer.minimize(() => {
      ...
      const pred: tf.Tensor = predict(xs);
      return loss(pred, ys);
    });
